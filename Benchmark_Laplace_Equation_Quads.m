function Benchmark_Laplace_Equation_Quads
...
for n=1:Nbenchmark
    tic
    Calculate_Integrals_Laplace_Quads_Midpoints(P,M_Quads,Con);
    tn_midpoints(n)=toc;

    tic
    Calculate_Integrals_Laplace_Quads_Vertices(P,Con);
    tn_vertices(n)=toc;
...
fprintf('Average time needed per runs using inner points: t=(%1.3f+-%1.3f) ms\n',t_mean_Midpoints*1e3,t_std_Midpoints*1e3)
fprintf('Average time needed per runs using vertices:     t=(%1.3f+-%1.3f) ms\n',t_mean_Vertices*1e3,t_std_Vertices*1e3)
fprintf('Average number of elements per second using midpoints: %1.3d\n',num_quads_per_sec_Midpoints)
